...
int main() {
	srand(time(NULL));
	MutatorState *mutator_state = newMutatorState();

	char test_expr[] = "x0|(x0*x0-(((x0^x0)/x0))";
	trimExpr(test_expr);
	char *cleaned_expr = removeUnmatchedParentheses(test_expr);

	Node *tree = parseExpr(cleaned_expr, cleaned_expr + strlen(cleaned_expr));	
	countOpndUses(tree, mutator_state);	
	restoreUnusedOpnds(tree, mutator_state);
	
	int expr_tree_strln;	
	char *expr_tree_as_str = exprTreeToString(tree, 16, &expr_tree_strln);		
	printf("%s\n", expr_tree_as_str);	

	for (int i = 0; i < 1; i++) {
		free(expr_tree_as_str);
		randomBranchSwap(tree, 40, 50, 3);
		randomReplaceOptrs(tree);
		
		expr_tree_as_str = exprTreeToString(tree, 16, &expr_tree_strln);	
		printf("%s\n", expr_tree_as_str);	
	}
	...
	free(cleaned_expr);
	freeExprTree(&tree);
	freeMutatorState(&mutator_state);
	return 0;
}
